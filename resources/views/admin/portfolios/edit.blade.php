                    ...
                    <form action="{{ route("admin.portfolios.update", [$portfolio->id]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('text_colour') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Portfolio::TEXT_COLOUR_RADIO as $key => $label)
                                <div>
                                    <input id="text_colour_{{ $key }}" name="text_colour" type="radio" value="{{ $key }}" {{ old('text_colour', $portfolio->text_colour) === (string)$key ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('pinned') ? 'has-error' : '' }}">
                            ...
                            <input value="1" type="checkbox" id="pinned" name="pinned" {{ (isset($portfolio) && $portfolio->pinned) || old('pinned', 0) === 1 ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('type') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Portfolio::TYPE_RADIO as $key => $label)
                                <div>
                                    <input id="type_{{ $key }}" name="type" type="radio" value="{{ $key }}" {{ old('type', $portfolio->type) === (string)$key ? 'checked' : '' }} required>
